...
const columns = [
  { label:'Rank', key:'rank' },
  { label:'Product / Design', key:'name' },
  { label:'Designer', key:'designer' },
  { label:'Orders', key:'orderCount' },
  { label:'Units Sold', key:'totalQty' },
  { label:'Gross Revenue', key:'revenueStr' },
];

export default function MasterReportProducts() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(`/api/reports/top-products?from=${dateFrom}&to=${dateTo}`);
  ...
  const totalUnits = products.reduce((s, p) => s + (Number(p.totalQty) || 0), 0);
  const topProduct = products[0];

  const tableData = products.map((p, i) => ({
    rank: i + 1,
    name: p.name || 'Untitled Product',
    designer: p.designerUsername ? `@${p.designerUsername}` : '-',
    orderCount: p.orderCount || 0,
    totalQty: p.totalQty || 0,
    revenueStr: fmt(p.totalRevenue),
    rawRevenue: p.totalRevenue || 0
  }));
    ...
    <div style={{ padding:'24px', maxWidth:'1200px', margin:'0 auto', fontFamily:'sans-serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'24px' }}>
        <div>
          ...
          <h1 style={{ fontSize:'28px', fontWeight:700, color:'#1a1a1a', margin:0 }}>Top Products & Designs</h1>
          <p style={{ color:'#666', margin:'4px 0 0', fontSize:'14px' }}>Best-selling items, quantities sold & product demand</p>
        </div>
        <button style={btn} onClick={() => exportCSV(tableData, 'products-report.csv', columns)}>⬇ Export CSV</button>
      </div>
      ...
      <div style={{ display:'flex', gap:'16px', flexWrap:'wrap', marginBottom:'28px' }}>
        <div style={card}><div style={lbl}>Distinct Items Sold</div><p style={val}>{products.length}</p></div>
        <div style={card}><div style={lbl}>Total Units Sold</div><p style={val}>{totalUnits}</p></div>
        <div style={card}><div style={lbl}>Total Product Revenue</div><p style={val}>{fmt(totalRevenue)}</p></div>
        <div style={card}><div style={lbl}>Top Performing Item</div><p style={{ ...val, fontSize:'16px' }}>{topProduct ? topProduct.name : '-'}</p></div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background:'#fff', border:'1px solid #E5E5E5', borderRadius:'12px', overflow:'hidden' }}>
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {tableData.length === 0 ? (
                <tr><td colSpan={columns.length} style={{ ...td, textAlign:'center', padding:'40px', color:'#666' }}>No product sales recorded in this period.</td></tr>
              ) : tableData.map((row, i) => (
                <tr key={i} style={{ background: i%2===0?'#fff':'#FAFAF8' }}>
                  <td style={{ ...td, fontWeight:700, color: i===0?'#C5A059':'#1a1a1a' }}>#{row.rank}</td>
                  <td style={{ ...td, fontWeight:500 }}>{row.name}</td>
                  <td style={{ ...td, color:'#666' }}>{row.designer}</td>
                  <td style={td}>{row.orderCount}</td>
                  <td style={td}>{row.totalQty}</td>
                  <td style={{ ...td, fontWeight:600 }}>{row.revenueStr}</td>
                </tr>
              ))}
            </tbody>
          </table>
