...
export default function MasterReportSupport() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await apiFetch(`/api/reports/support-tickets?from=${dateFrom}&to=${dateTo}`);
  ...
  const tableData = tickets.map(t => ({
    ...
    createdDate: t.createdAt ? new Date(t.createdAt).toLocaleDateString('en-IN') : '-'
  }));
    ...
    <div style={{ padding:'24px', maxWidth:'1200px', margin:'0 auto', fontFamily:'sans-serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'24px' }}>
        <div>
          ...
          <h1 style={{ fontSize:'28px', fontWeight:700, color:'#1a1a1a', margin:0 }}>Customer Support & Tickets</h1>
          <p style={{ color:'#666', margin:'4px 0 0', fontSize:'14px' }}>Ticket volume, resolution rates & open issues tracking</p>
        </div>
        <button style={btn} onClick={() => exportCSV(tableData, 'support-tickets-report.csv', columns)}>⬇ Export CSV</button>
      </div>
      ...
      <div style={{ display:'flex', gap:'16px', flexWrap:'wrap', marginBottom:'28px' }}>
        <div style={card}><div style={lbl}>Total Tickets</div><p style={val}>{summary.total ?? 0}</p></div>
        <div style={card}><div style={lbl}>Open Tickets</div><p style={{ ...val, color:'#d97706' }}>{summary.open ?? 0}</p></div>
        <div style={card}><div style={lbl}>Resolved / Closed</div><p style={{ ...val, color:'#15803d' }}>{(summary.resolved ?? 0) + (summary.closed ?? 0)}</p></div>
        <div style={card}><div style={lbl}>Resolution Rate</div><p style={val}>{summary.total > 0 ? `${Math.round((((summary.resolved ?? 0) + (summary.closed ?? 0)) / summary.total) * 100)}%` : '100%'}</p></div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ background:'#fff', border:'1px solid #E5E5E5', borderRadius:'12px', overflow:'hidden' }}>
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {tableData.length === 0 ? (
                <tr><td colSpan={columns.length} style={{ ...td, textAlign:'center', padding:'40px', color:'#666' }}>No support tickets filed in this period.</td></tr>
              ) : tableData.map((row, i) => {
                const s = row.status.toLowerCase();
                const isResolved = s.includes('resolved') || s.includes('closed');
                const isOpen = s.includes('open') || s.includes('progress');
                return (
                  <tr key={i} style={{ background: i%2===0?'#fff':'#FAFAF8' }}>
                    <td style={{ ...td, fontFamily:'monospace', fontWeight:600 }}>{row.id}</td>
                    <td style={{ ...td, fontWeight:500 }}>{row.subject}</td>
                    <td style={td}>{row.userName}</td>
                    <td style={td}>
                      <span style={{ 
                        padding:'3px 8px', borderRadius:'99px', fontSize:'12px', fontWeight:600,
                        background: isResolved ? '#dcfce7' : isOpen ? '#fef3c7' : '#f3f4f6',
                        color: isResolved ? '#15803d' : isOpen ? '#b45309' : '#374151'
                      }}>
                        {row.status}
                      </span>
                    </td>
