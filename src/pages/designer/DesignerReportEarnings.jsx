...
export default function DesignerReportEarnings() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // Explode items per order where designer earned
                const earnedItems = [];
                (ordersData || []).forEach(o => {
                    (o.items || []).forEach(item => {
                        if (item.designer_earnings && Number(item.designer_earnings) > 0) {
                            earnedItems.push({
                                order_id: o.id,
                                created_at: o.created_at,
                                status: o.status,
                                design_name: item.design_name || 'Design',
                                earnings: Number(item.designer_earnings)
                            });
                        }
                    });
                });
                
                earnedItems.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
                setOrders(earnedItems);
                setWallet(walletData || { balance: 0, pending_balance: 0, total_withdrawn: 0, total_earnings: 0 });
            } catch (err) {
    ...
    const monthTotals = filteredOrders.reduce((acc, o) => {
        const month = new Date(o.created_at).toISOString().slice(0, 7); // YYYY-MM
        acc[month] = (acc[month] || 0) + o.earnings;
        return acc;
    }, {});
    const maxMonth = Math.max(...Object.values(monthTotals), 1);

    const handleExport = () => {
        exportCSV(filteredOrders, 'designer_earnings.csv', [
            { label: 'Order ID', key: 'order_id' },
            { label: 'Date', key: 'created_at' },
            { label: 'Design', key: 'design_name' },
    ...
    return (
        <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto', fontFamily: 'sans-serif' }}>
            ...
            <h1 style={{ color: '#1a1a1a', marginBottom: '20px' }}>Earnings History</h1>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '15px', marginBottom: '30px' }}>
                <div style={{ background: '#FAFAF8', padding: '15px', borderRadius: '8px', border: '1px solid #E5E5E5' }}>
                    <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '5px' }}>Total Earned (All Time)</div>
                    <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#1a1a1a' }}>{fmt(wallet?.total_earnings)}</div>
                </div>
                <div style={{ background: '#FAFAF8', padding: '15px', borderRadius: '8px', border: '1px solid #E5E5E5' }}>
                    <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '5px' }}>Current Balance</div>
                ...
                <div style={{ background: '#FAFAF8', padding: '15px', borderRadius: '8px', border: '1px solid #E5E5E5' }}>
                    <div style={{ color: '#666', fontSize: '0.9rem', marginBottom: '5px' }}>Pending Payout</div>
                    <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#C5A059' }}>{fmt(wallet?.pending_balance)}</div>
                </div>
            </div>

            <div style={{ marginBottom: '30px' }}>
                <h3 style={{ color: '#1a1a1a', marginBottom: '10px' }}>Monthly Earnings</h3>
                <div style={{ display: 'flex', alignItems: 'flex-end', height: '150px', gap: '10px', background: '#FAFAF8', padding: '15px', borderRadius: '8px', border: '1px solid #E5E5E5' }}>
                    {Object.keys(monthTotals).length === 0 ? (
                        <div style={{ color: '#666', width: '100%', textAlign: 'center', alignSelf: 'center' }}>No earnings this period</div>
                    ) : (
                        Object.entries(monthTotals).sort().map(([month, amount]) => {
                            const height = `${(amount / maxMonth) * 100}%`;
                            return (
                                <div key={month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }}>
                                    <div style={{ fontSize: '0.7rem', color: '#666', marginBottom: '4px' }}>{fmt(amount)}</div>
                                    <div style={{ width: '100%', background: '#C5A059', height, borderRadius: '4px 4px 0 0', minHeight: '4px' }}></div>
                                    <div style={{ fontSize: '0.75rem', marginTop: '5px', color: '#1a1a1a' }}>{month}</div>
                                </div>
                            );
                        })
                    )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead>
                    <tr style={{ borderBottom: '2px solid #E5E5E5' }}>
                        ...
                        <th style={{ padding: '12px', color: '#666' }}>Design</th>
                ...
                <tbody>
                    {filteredOrders.length === 0 ? (
                        ...
                        filteredOrders.map((o, idx) => (
                            <tr key={`${o.order_id}-${idx}`} style={{ borderBottom: '1px solid #E5E5E5' }}>
                                <td style={{ padding: '12px' }}>{o.order_id}</td>
                                <td style={{ padding: '12px' }}>{new Date(o.created_at).toLocaleDateString('en-IN')}</td>
                                <td style={{ padding: '12px' }}>{o.design_name}</td>
                                <td style={{ padding: '12px', fontWeight: 'bold' }}>{fmt(o.earnings)}</td>
