...
function UserTracking() {
            ...
            try {
                if (orderIdParam) {
                    ...
                    if (data && data.length > 0) {
                        // Filter for live orders (pending, confirmed, manufacturing, shipping)
                        const live = data.filter(o => ['pending', 'confirmed', 'manufacturing', 'shipping'].includes(o.status));
                        if (live.length > 0) {
                            setOrders(live);
                        } else {
                            // Fallback to the latest order if no live orders exist
                            setOrders([data[0]]);
                        }
                    } else {
                        setOrders([]);
    ...
    const parseTrackingInfo = (trackingStr) => {
        if (!trackingStr) return null;
        const match = trackingStr.match(/^\[([^\]]+)\]\s*(.*)$/);
        if (match) {
            return { partner: match[1].trim(), id: match[2].trim() };
        }
        return { partner: null, id: trackingStr };
    };
                    ...
                    {loading ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                            {orders.map(order => {
                                ...
                                const tracking = parseTrackingInfo(order.tracking_id);
                                        ...
                                        <div className="order-details-summary">
                                            ...
                                            {tracking && (
                                                <>
                                                    {tracking.partner && (
                                                        <div className="details-row">
                                                            <span className="details-label">Shipping Partner:</span>
                                                            <span className="details-value" style={{ fontWeight: 600 }}>{tracking.partner}</span>
                                                        </div>
                                                    )}
                                                    <div className="details-row">
                                                        <span className="details-label">Tracking ID:</span>
                                                        <span className="details-value" style={{ fontWeight: 700, color: 'var(--gold)', letterSpacing: '0.5px' }}>{tracking.id}</span>
                                                    </div>
                                                </>
                                            )}
